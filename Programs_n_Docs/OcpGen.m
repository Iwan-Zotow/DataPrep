function  OcpGen(RadiationUnitType, OuterCupType)
    ...
    for segment = 1 : length(OcpParam.FC)
        ...
        fid = fopen(fiducialsegtxtfilename, 'wt', 'native', 'US-ASCII');
        if fid == -1
    ...
    fid = fopen(insidewalltxtfilename, 'wt', 'native', 'US-ASCII');
    if fid == -1
    ...
    fid = fopen(outsidewalltxtfilename, 'wt', 'native', 'US-ASCII');
    if fid == -1
